...
def main():
    ...
    parser.add_argument('--dataset', type=Path, 
                        default=Path("/proj/vlarsson/datasets/megadepth/Undistorted_SfM"))
    parser.add_argument('--outputs', type=Path, 
                        default=Path("/proj/vlarsson/outputs/query_sets"))
    parser.add_argument('--scene', type=str, default=None, help="Process a single scene ID")
    ...
    for scene in tqdm(scenes):
        try:
            extract_query_sets(
                scene, args.dataset, depth_root, args.outputs,
                sample_ratio=args.sample_ratio,
                query_image_ratio=args.query_ratio
            )
