...
def evaluate_pose(matched_2d, matched_3d, camera, max_error): 
    if len(matched_2d) < 4:
        return None

    # Reconstruct the PyCOLMAP Camera object
    ...
    # Configure RANSAC and Refinement using the custom max_error argument
    estimation_options = {"ransac": {"max_error": max_error}}
    refinement_options = {"refine_focal_length": True, "refine_extra_params": True}
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate Pose Accuracy over multiple Scenes")
    ...
    logger.info(f"Starting Pose Evaluation for {len(scenes)} scenes using {method} (RANSAC max_error: {args.max_error}px)")

    # Initialize Matchers Based on Method
    ...
    for scene in scenes:
        ...
        for query_name in tqdm(queries, desc=f"Evaluating {scene}"):
            ...
            if not primary_ref:
                failed_pnp_count += 1
                all_t_errors.append(np.inf)
                all_r_errors.append(np.inf)
                continue

            # Prepare HLOC references from covisibility
            top_refs = []
            if method == "HLOC":
                # Get the exact COLMAP image IDs that passed the pruning threshold
                ...
                # Ensure the primary reference is always included and processed first
                if primary_ref in top_refs:
                    top_refs.remove(primary_ref)
                top_refs.insert(0, primary_ref)

                # logger.info(f"Query: {query_name} | Primary Ref: {primary_ref} | HLOC Refs: {top_refs}")

            # Load basic query features
            ...
            if pose_res is not None:
                all_t_errors.append(pose_res["t_error"])
                all_r_errors.append(pose_res["r_error_deg"])
            else:
                failed_pnp_count += 1
                all_t_errors.append(np.inf)
                all_r_errors.append(np.inf)
    ...
    # Print metrics
    log_metrics(all_t_errors, all_r_errors, failed_pnp_count, total_queries_evaluated, method)
